...
<head>
    ...
    <style>
        body {
            ...
            height: 220vh;
            background-image: url('/img/BackgroundWelcom.png');
...
.animation-text {
    ...
    position: absolute;
    top: 880px; /* Ajustez cette valeur pour déplacer le texte vers le bas */
    left: 50%;
...
@media screen and (min-width: 1900px) {
    ...
    border-right: 2px solid #333;
    position: absolute;
    top: 120vh; /* Utilisez vh pour un positionnement relatif à la hauteur de la vue */
    left: 50%;
    transform: translate(-50%, -50%);
    color: black;
}
...
<body>
<img src="/img/Rejoint.png" alt="Description de l'image" class="position-Rejoint ">
<div id="app">
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js"></script>
<img src="/img/texte.png" alt="Description de l'image" class="position-text">
<img src="/img/logo.png" alt="Description de l'image" class="position-logo">
...
<div class="animation-text"> <span class="number-animation" id="userCount">0</span> Utilisateurs nous font déjà confiance!</div>
...
<script>
    ...
    const increment = Math.ceil(target / (duration / 20)); // 20 steps per second
    let count = 0;

    function animateCount() {
        const interval = setInterval(() => {
            count = Math.min(count + increment, target);
            userCountSpan.textContent = count.toLocaleString();
            if (count >= target) {
...
<script>
 window.addEventListener('load', function() {
    const image = document.querySelector('.position-Rejoint');
